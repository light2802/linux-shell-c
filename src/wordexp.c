...
struct word_s *word_expand(char *orig_word)
{
    ...
    char *pstart = (char*)malloc(strlen(orig_word)+1);
    if(!pstart)
    {
        return NULL;
    }
...
char *command_substitute(char *orig_cmd)
{
     ...
     */
    char *cmd = (char*)malloc(strlen(orig_cmd+1));
    ...
    while((i = fread(b, 1, 1024, fp)))
    {
        ...
        {
            char *buf2 = (char*)realloc(buf, bufsz+i+1);
